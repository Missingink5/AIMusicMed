...
1. **本地音乐库优先**: 根据用户情绪智能选择本地音乐文件（**需要您手动添加音乐**）
2. **AI生成备选**: 当本地音乐不足时，自动回退到AI音乐生成

**重要说明**: 本地音乐库中的所有音乐文件需要您手动添加，系统不会自动生成任何音乐文件。这确保了音乐的质量和您的个人偏好。
...
# 查看音乐文件添加说明
# 脚本会提供详细的文件添加指南，不会自动创建任何文件
```

**重要**: 在测试之前，请先手动添加音乐文件到对应的情绪目录中。

### 2. 添加真实音乐文件

**这是必需步骤** - 在每个情绪目录中手动添加音乐文件：

```bash
music_library/
├── Quiet/
│   ├── meditation_01.mp3        # 您添加的冥想音乐
│   ├── nature_sounds.wav        # 您添加的自然声音
│   └── calm_piano.mp3           # 您添加的钢琴曲
├── Happy/
│   ├── upbeat_01.mp3            # 您添加的欢快音乐
│   └── happy_melody.wav         # 您添加的快乐旋律
# ... 其他情绪目录，都需要您手动添加音乐
```
...
**推荐音乐文件规格**：
- 时长：30秒 - 3分钟（系统会自动循环或截取）
- 格式：MP3, WAV, M4A（支持大多数音频格式）
- 每个情绪：5-20首音乐（建议至少5首保证多样性）
- 总容量：建议每个目录50MB以内
- 质量：选择您认为真正有助于情绪调节的音乐
...
```
🎵 启动智能音乐选择系统...
📚 发现本地音乐库: 25 首音乐 (您手动添加的音乐)
📊 音乐分布: {'平静': 8, '喜悦': 5, '忧郁': 3, ...}
🎼 使用本地音乐库生成背景音乐...
  ✓ 片段 1: 使用本地音乐 your_music_01.mp3
  ✓ 片段 2: 使用本地音乐 your_music_02.wav
✅ 使用本地音乐库
```
...
**Q: 如果某个情绪没有音乐文件怎么办？**
A: 系统会自动回退到AI生成音乐，确保始终有背景音乐。建议为每个情绪目录至少添加5首音乐。
...
**Q: 如何获得适合的音乐？**
A: 建议使用您的个人收藏、免版权音乐、或购买正版音乐。避免使用有版权争议的音乐。

**Q: AI音乐生成失败怎么办？**
A: 系统会自动创建静音文件，保证应用正常运行。这也是为什么建议添加本地音乐的原因。
