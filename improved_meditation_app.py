        ...
        try:
            ...
            if "audioop" in str(e):
                raise MeditationAppError(
                    "检测到 audioop 模块问题，请使用 Python 3.13 兼容版本: run_py313_app.py"
                )
...
"""
    ...
    async def generate_speech(self, script_prompts: List[Dict]) -> List[str]:
        ...
        for i, segment in enumerate(script_prompts):
            file_path = os.path.join(self.config.paths.temp_dir, f"speech_{i:02d}.mp3")
    ...
    def combine_audio(self, speech_files: List[str], music_files: List[str]) -> str:
        ...
        for i, (speech_file, music_file) in enumerate(zip(speech_files, music_files)):
            try:
                ...
                if len(music) < len(speech):
                    padding = AudioSegment.silent(duration=len(speech) - len(music))
                    music = music + padding
                else:
        ...
        final_audio.export(
            output_path, 
            format="mp3", 
            bitrate=self.config.audio.output_bitrate
        )
